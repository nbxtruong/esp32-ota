...
void networkConnect(const char *ssid, const char *password)
{
    ...
    memcpy(wlanStaConfig.sta.ssid, ssid, sizeof(wlanStaConfig.sta.ssid) / sizeof(char));
    memcpy(wlanStaConfig.sta.password, password, sizeof(wlanStaConfig.sta.password) / sizeof(char));
        ...
        do
        {
            ...
            if (n > 0)
            {
                ...
                if (totalLen > 0)
                {
                    ...
                    if ((recordWithLengthIndicator && totalLen == recordLen) || (!recordWithLengthIndicator && buf[totalLen - 1] == '\n'))
                    {
